        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          {step === 1 && (
            <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="bg-[#0f172a]/60 border border-white/10 rounded-2xl p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <input className="input" placeholder="Nombre" value={contact.name} onChange={(e)=>setContact({...contact, name:e.target.value})} />
                <input className="input" placeholder="Apellido" value={contact.surname} onChange={(e)=>setContact({...contact, surname:e.target.value})} />
                <input className="input md:col-span-2" placeholder="Email" value={contact.email} onChange={(e)=>setContact({...contact, email:e.target.value})} />
                <input className="input md:col-span-2" placeholder="Teléfono" value={contact.phone} onChange={(e)=>setContact({...contact, phone:e.target.value})} />
              </div>
              <h3 className="font-semibold">Dirección de envío</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <input className="input md:col-span-2" placeholder="Calle y número" value={address.street} onChange={(e)=>setAddress({...address, street:e.target.value})} />
                <input className="input" placeholder="Ciudad" value={address.city} onChange={(e)=>setAddress({...address, city:e.target.value})} />
                <input className="input" placeholder="Código Postal" value={address.postalCode} onChange={(e)=>setAddress({...address, postalCode:e.target.value})} />
                <input className="input" placeholder="Provincia" value={address.state || ''} onChange={(e)=>setAddress({...address, state:e.target.value})} />
                <input className="input" placeholder="País" value={address.country} onChange={(e)=>setAddress({...address, country:e.target.value})} />
              </div>
          ...
          {step === 3 && (
            <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="bg-[#0f172a]/60 border border-white/10 rounded-2xl p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <input className="input md:col-span-2" placeholder="Número de tarjeta" value={card.number} onChange={(e)=>setCard({...card, number:e.target.value})} />
                <input className="input" placeholder="Nombre del titular" value={card.name} onChange={(e)=>setCard({...card, name:e.target.value})} />
                <input className="input" placeholder="MM/AA" value={card.expiry} onChange={(e)=>setCard({...card, expiry:e.target.value})} />
